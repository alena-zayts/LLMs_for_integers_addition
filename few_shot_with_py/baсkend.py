...
class MyProgramInterface:
    ...
    def __init__(self, model: str, stop: str = '\n\n\n', get_answer_expr: str = None, verbose: bool = False) -> None:
        self.model = model
        self.stop = stop
    ...
    def generate(self, prompt: str, temperature: float, top_p: float, max_tokens: int, majority_at: int):
    ...
    def run(self, prompt: str, time_out: int = 10, temperature: float = 0.0, top_p: float = 1.0,
...
# GPT-3 API
def call_gpt(prompt, model, stop, temperature, top_p, max_tokens, majority_at):
    ...
    for i in range(20 * (num_completions // num_completions_batch_size + 1)):
        try:
            ...
            if model.startswith('gpt-4') or model.startswith('gpt-3.5-turbo'):
                ans = chat_api(model, max_tokens, stop, prompt, temperature, top_p, requested_completions,
                               requested_completions)
            else:
                ans = completions_api(model, max_tokens, stop, prompt, temperature, top_p, requested_completions,
                                      requested_completions)
...
def chat_api(model, max_tokens, stop, prompt, temperature, top_p, n, best_of):
    ans = openai.ChatCompletion.create(
        ...
        messages=[
            {'role': 'system',
             'content': 'You are a helpful assistant that can write Python code that solves the problem of adding '
                        'numbers similarly to the examples that you will be provided.'},
